...
if __name__ == "__main__":
    ...
    for i in words:
        if i:
            if i[0] == '"' and i[-1] == '"':
                query_params.append(
                    '?s fts:match "' + Tracker.sparql_escape_string(i[1:-1]) + '"'
                )
            else:
                query_params.append(
                    '?s fts:match "' + Tracker.sparql_escape_string(i) + '*"'
                )
    query = (
        """SELECT ?s nie:url(?s) nmm:musicAlbum(?s) nmm:performer(?s) nmm:trackNumber(?s) nie:title(?s) nie:mimeType(?s) rdf:type(?s)
                 WHERE { """
        + " . ".join(query_params)
        + """ }
