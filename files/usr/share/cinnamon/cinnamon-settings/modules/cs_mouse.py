...
class Module:
    ...
    def __init__(self, content_box):
        ...
        sidePage = SidePage(
            _("Mouse and Touchpad"), "cs-mouse", keywords, content_box, module=self
        )
    ...
    def on_module_selected(self):
        if not self.loaded:
            ...
            switch = GSettingsSwitch(
                _("Left handed (mouse buttons inverted)"),
            ...
            switch = GSettingsSwitch(
                _("Show position of pointer when the Control key is pressed"),
                "org.cinnamon.desktop.peripherals.mouse",
                "locate-pointer",
            )
            settings.add_row(switch)
            ...
            switch = GSettingsSwitch(
                _("Emulate middle click by clicking both left and right buttons"),
                "org.cinnamon.desktop.peripherals.mouse",
            ...
            switch = GSettingsSwitch(
                _("Paste the current selection when middle-click is pressed"),
            ...
            revealer = SettingsRevealer(
                "org.cinnamon.desktop.peripherals.touchpad",
                "send-events",
                values=("enabled", "disabled-on-external-mouse"),
            )
...
class ScrollMethodCombo(ComboBox):
    def __init__(self):
        ...
        super(ScrollMethodCombo, self).__init__(
            _("Scrolling method"), options=self.scrolling_list, valtype=int
        )

        self.touchpad_settings = Gio.Settings(
            schema_id="org.cinnamon.desktop.peripherals.touchpad"
        )
        self.touchpad_settings.connect(
            "changed::two-finger-scrolling-enabled", self.on_setting_changed
        )
        self.touchpad_settings.connect(
            "changed::edge-scrolling-enabled", self.on_setting_changed
        )
