...
class DeskletsViewSidePage(SidePage):
    ...
    def load(self, window):
        ...
        line1 = Gtk.Label()
        line1.set_markup(f"<i><small>{_('Note: Some desklets require the border/header to be always present')}</small></i>")
        line1.get_style_context().add_class("dim-label")
        widget.pack_start(line1, True, True, 0)
        line2 = Gtk.Label()
        line2.set_markup(f"<i><small>{_('Such requirements override the settings selected here')}</small></i>")
