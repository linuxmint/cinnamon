...
class Module:
    ...
    def onOverAmplificationOutChanged(self, settings=None, key=None):
        overamplification_out = self.sound_settings.get_boolean(OVERAMPLIFICATION_OUT_KEY)
        self.outVolume.slider.clear_marks()
        if overamplification_out:
    ...
    def onOverAmplificationInChanged(self, settings=None, key=None):
        overamplification_in = self.sound_settings.get_boolean(OVERAMPLIFICATION_IN_KEY)
        self.inVolume.slider.clear_marks()
        if overamplification_in:
