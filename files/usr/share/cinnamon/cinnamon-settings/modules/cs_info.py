...
def getDiskSize():
    ...
    try:
        out = getProcessOut(
            ("lsblk", "--json", "--output", "size", "--bytes", "--nodeps")
        )
...
def createSystemInfos():
    ...
    try:
        ...
        memunit = ""
    processorName = (
        procInfos["cpu_name"].replace("(R)", "\u00a9").replace("(TM)", "\u2122")
    )
    ...
    if memunit == "kB":
        infos.append(
            (_("Memory"), "%.1f %s" % ((float(memsize) / (1024 * 1024)), _("GiB")))
        )
    else:
    ...
    try:
        infos.append(
            (diskText, "%.1f %s" % ((diskSize / (1000 * 1000 * 1000)), _("GB")))
        )
    except:
...
class Module:
    ...
    def __init__(self, content_box):
        keywords = _(
            "system, information, details, graphic, sound, kernel, version, about"
        )
        sidePage = SidePage(
            _("System Info"), "cs-details", keywords, content_box, module=self
        )
    ...
    def on_module_selected(self):
        if not self.loaded:
                ...
                try:
                    if "/" in systemIcon:
                        pixbuf = GdkPixbuf.Pixbuf.new_from_file_at_scale(
                            systemIcon, -1, 100, True
                        )
                        systemIcon = Gtk.Image.new_from_pixbuf(pixbuf)
                    else:
                        systemIcon = Gtk.Image.new_from_icon_name(
                            systemIcon, Gtk.IconSize.DIALOG
                        )
            ...
            if shutil.which("upload-system-info"):
                ...
                button = Gtk.Button(
                    label=_("Upload system information"),
            ...
            if shutil.which("inxi"):
                ...
                spinner = Gtk.Spinner(visible=True)
                button = Gtk.Button(
                    label=_("Copy to clipboard"), always_show_image=True, image=spinner
                )
                button.connect(
                    "clicked", self.on_copy_clipboard_button_clicked, spinner
                )
                widget.pack_start(button, True, True, 0)
                settings.add_row(widget)
    ...
    def on_copy_clipboard_button_clicked(self, button, spinner):
        ...
        def _run_inxi(spinner):
            ...
            try:
                inxiOutput = subprocess.run(
                    ["inxi", "-FJxxxrzc0"], check=True, stdout=subprocess.PIPE
                ).stdout
            except Exception as e:
                print(
                    "An error occurred while copying the system information to clipboard"
                )
                print(e)
    ...
    def on_upload_button_clicked(self, button, spinner):
        try:
            subproc = Gio.Subprocess.new(
                ["upload-system-info"], Gio.SubprocessFlags.NONE
            )
