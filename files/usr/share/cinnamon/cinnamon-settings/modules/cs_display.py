...
class Module:
    ...
    def __init__(self, content_box):
        ...
        self.sidePage = SidePage(
            _("Display"), "cs-display", keywords, content_box, 650, module=self
        )
    ...
    def on_module_selected(self):
        if not self.loaded:
            ...
            switch.set_tooltip_text(
                _(
                    "Select this option to disable automatic screen rotation on hardware equipped with supported accelerometers."
                )
            )
            ...
            switch.set_tooltip_text(
                _(
                    "Select this option to display additional layout controls for per-monitor scaling."
                )
            )
            settings.add_row(switch)
            ...
            self.experimental_features_changed(
                self.muffin_settings, "x11-randr-fractional-scaling"
            )
            self.muffin_settings.connect(
                "changed::experimental-features", self.experimental_features_changed
            )
            self.fractional_switch.connect(
                "notify::active", self.fractional_switch_toggled
            )

            scale_mode_options = [
                ["scale-up", _("Scale content up")],
                ["scale-ui-down", _("Scale content down")],
            ]

            widget = GSettingsComboBox(
                _("Fractional scaling mode"),
            ...
            settings.add_reveal_row(
                ...
                [],
                lambda val, vals: "x11-randr-fractional-scaling" in val,
            )
    ...
    def experimental_features_changed(self, settings, key):
        ...
        self.fractional_switch.set_active(
            set(FRACTIONAL_ENABLE_OPTIONS).issubset(features)
        )
