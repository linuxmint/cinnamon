...
removable_media_defs = [
    ...
    (
        ...
        _("Music player"),
        _("Select an application to run when a music player is connected"),
    ),
    (
        "x-content/image-dcf",
        _("Photos"),
        _("Select an application to run when a camera is connected"),
    ),
    (
        "x-content/unix-software",
        _("Software"),
        _("Select an application for software CDs"),
    ),
]
...
class ButtonTable(Gtk.Table):
    ...
    def addRow(self, label, button):
        ...
        self.attach(
            button, 1, 2, self.row, self.row + 1, Gtk.AttachOptions.FILL, 0, 0, 0
        )
        self.row += 1
        ...
        if info:
            ...
            if self.content_type == "x-scheme-handler/http":
                if not info.set_as_default_for_type("x-scheme-handler/https"):
                    print(
                        f"  Failed to set '{info.get_name()}' as the default application for 'x-scheme-handler/https'"
                    )
    ...
    def __init__(self):
        ...
        while self.this_item is not None and count_up < len(apps):
            ...
            if cat_val is not None and "TerminalEmulator" in cat_val:
                ...
                if (
                    exec_val is not None
                    and name_val is not None
                    and icon_val is not None
                    and "gksu" not in exec_val
                    and exec_val not in self.active_items
                ):
                    self.append_custom_item(
                        exec_val, name_val, Gio.ThemedIcon.new(icon_val)
                    )
...
class TerminalExecArgEntry(Gtk.Entry):
    def __init__(self):
        ...
        self.set_tooltip_text(
            _("Command-line option for your terminal to execute a passed-in command.")
        )
...
class DefaultCalculatorButton(Gtk.AppChooserButton):
    def __init__(self):
        ...
        while self.this_item is not None and count_up < len(apps):
            ...
            if (
                (cat_val is not None and "Calculator" in cat_val)
                or (exec_val is not None and "alculator" in exec_val.lower())
                or (name_val is not None and "alculator" in name_val.lower())
                or (comment_val is not None and "alculator" in comment_val.lower())
            ):
...
class OtherTypeDialog(Gtk.Dialog):
    def __init__(self, media_settings, transient_parent):
        super(OtherTypeDialog, self).__init__(
            title=_("Other Media"),
        ...
        self.vbox.pack_start(
            ColumnBox(_("Select how other media should be handled"), table),
            True,
            True,
            6,
        )
    ...
    def onTypeComboChanged(self, type_combo):
        ...
        self.application_combo = CustomAppChooserButton(
            self.media_settings, x_content_type, heading
        )
    ...
    def on_module_selected(self):
        if not self.loaded:
            ...
            self.other_type_dialog = OtherTypeDialog(
                self.media_settings, self.sidePage.window
            )
            ...
            self.sidePage.stack.add_titled(
                page, "preferred", _("Preferred applications")
            )
            ...
            for name in preferred_app_defs:
                ...
                if len(items) > 0:
                    ...
                    for item in items:
                        ...
                        button = DefaultAppChooserButton(
                            item[PREF_CONTENT_TYPE], item[PREF_GEN_CONTENT_TYPE]
                        )
                    ...
                    if name.lower() == "system":
                        ...
                        entry_box = Gtk.Box(
                            orientation=Gtk.Orientation.HORIZONTAL, margin_top=6
                        )
            ...
            switch.label.set_markup(
                "<b>%s</b>" % _("Prompt or start programs on media insertion")
            )
            switch.fill_row()
            page.add(switch)
            ...
            for d in removable_media_defs:
                widget = SettingsWidget()
                button = CustomAppChooserButton(
                    self.media_settings, d[DEF_CONTENT_TYPE], d[DEF_HEADING]
                )
