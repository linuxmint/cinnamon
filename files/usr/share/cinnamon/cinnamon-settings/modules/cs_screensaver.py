...
class Module:
    ...
    def __init__(self, content_box):
        ...
        sidePage = SidePage(
            _("Screensaver"), "cs-screensaver", keywords, content_box, module=self
        )
    ...
    def on_module_selected(self):
        ...
        widget = GSettingsComboBox(
            _("Delay before starting the screensaver"),
        ...
        widget = GSettingsSwitch(
            _("Lock the computer when put to sleep"),
        ...
        widget = GSettingsSwitch(
            _("Lock the computer after the screensaver starts"), schema, "lock-enabled"
        )
        widget.set_tooltip_text(
            _(
        ...
        widget.set_tooltip_text(
            _("Show the clock on the wallpaper instead of just on the unlock screen")
        )
        settings.add_row(widget)

        widget = GSettingsSwitch(
            _("Use a custom date and time format"), schema, "use-custom-format"
        )
        settings.add_row(widget)

        widget = GSettingsEntry(
            _("Time Format"), schema, "time-format", size_group=size_group
        )
        settings.add_reveal_row(widget, schema, "use-custom-format")

        widget = GSettingsEntry(
            _("Date Format: "), schema, "date-format", size_group=size_group
        )
        settings.add_reveal_row(widget, schema, "use-custom-format")
        ...
        widget = GSettingsEntry(
            _("Show this message when the screen is locked"), schema, "default-message"
        )
        widget.set_child_packing(
            widget.content_widget, True, True, 0, Gtk.PackType.START
        )
        widget.set_tooltip_text(
            _("This is the default message displayed on your lock screen")
        )
        settings.add_row(widget)

        settings.add_row(
            GSettingsFontButton(
                _("Font"), "org.cinnamon.desktop.screensaver", "font-message"
            )
        )

        widget = GSettingsSwitch(
            _("Ask for a custom message when locking the screen from the menu"),
            schema,
        ...
        widget = GSettingsSwitch(
            _("Allow keyboard shortcuts"), schema, "allow-keyboard-shortcuts"
        )
        widget.set_tooltip_text(
            _(
        ...
        widget = GSettingsSwitch(
            _("Show media player controls"), schema, "allow-media-control"
        )
        widget.set_tooltip_text(
            _("For compatible players, show playback controls while media is playing")
        )
        settings.add_row(widget)

        widget = GSettingsSwitch(_("Show album art"), schema, "show-album-art")
        widget.set_tooltip_text(
            _("If available, show album art while media is playing")
        )
        settings.add_row(widget)

        widget = GSettingsSwitch(_("Show info panel"), schema, "show-info-panel")
        widget.set_tooltip_text(
            _("Show the number of missed notifications and the battery status")
        )
        settings.add_row(widget)

        widget = GSettingsSwitch(
            _("Allow floating clock and album art widgets"), schema, "floating-widgets"
        )
        widget.set_tooltip_text(
            _(
