...
class Module:
    ...
    def __init__(self, content_box):
        ...
        self.sidePage = SidePage(
            _("Date & Time"), "cs-date-time", keywords, content_box, 560, module=self
        )
    ...
    def _on_proxy_ready(self):
        ...
        if self.zone is None:
            ...
            self.tz_selector.connect(
                "timezone-changed", self.on_selector_location_changed
            )
    ...
    def set_date_and_time(self, *args):
        unaware = datetime.datetime.combine(
            self.date_chooser.get_date(), self.time_chooser.get_time()
        )
...
class SytemdDBusProxyHandler(object):
    ...
    def set_ntp(self, active):
        if self._proxy:
            ...
            self._proxy.SetNTP(
                "(bb)", active, True, result_handler=async_empty_callback
            )
...
class TimeZoneSelector(SettingsWidget):
    __gsignals__ = {"timezone-changed": (GObject.SignalFlags.RUN_FIRST, None, (str,))}
    ...
    def on_city_changed(self, *args):
        self.timezone = "/".join(
            [self.region_combo.get_active_id(), self.city_combo.get_active_id()]
        )
