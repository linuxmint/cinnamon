        ...
        if len(self.jobs) > 0:
            ...
            with self.lock:
                ...
                newthread = threading.Thread(
                    target=self.thread_function_wrapper, args=job
                )
...
class Spice_Harvester(GObject.Object):
    ...
    def __init__(self, collection_type, window=None):
        ...
        self.cache_folder = os.path.join(
            GLib.get_user_cache_dir(), "cinnamon", "spices", self.collection_type
        )
        ...
        if self.themes:
            ...
            self.install_folder = (
                f"{home}/.local/share/cinnamon/{self.collection_type}s/"
            )
            self.spices_directories = (
                f"/usr/share/cinnamon/{self.collection_type}s/",
                self.install_folder,
            )
        ...
        self.monitor = None
        try:
            self.monitor = Gio.File.new_for_path(self.install_folder).monitor_directory(
                0, None
            )
    ...
    def _update_status(self, *args):
        try:
            if self._proxy and self._proxy.get_name_owner() and not self.actions:
                self.running_uuids = self._proxy.GetRunningXletUUIDs(
                    "(s)", self.collection_type
                )
            else:
                self.running_uuids = []
        except:
    ...
    def _update_progress(self, count, blockSize, totalSize):
        if self.download_manager.busy() and self.download_total_files > 1:
            ...
            fraction = (
                count * blockSize / float((totalSize / blockSize + 1) * blockSize)
            )
    ...
    def _advance_queue(self):
        ...
        if len(self.jobs) > 0:
            ...
            if self.monitor is not None:
                try:
                    self.monitorId = self.monitor.connect(
                        "changed", self._directory_changed
                    )
                except Exception as e:
                    # File monitors can fail when the OS runs out of file handles
                    print(e)
    ...
    def _download(self, out_file, url, binary=True):
        ...
        try:
            ...
            if (
                ...
                and not self.download_manager.abort_status
            ):
                self.errorMessage(
                    _(
                ...
                for uuid in extensions:
                    ...
                    if uuid.endswith(".nemo_action") and not os.path.exists(uuid_path):
                        ...
                        try:
                            keyfile.load_from_file(
                                full_path, GLib.KeyFileFlags.KEEP_TRANSLATIONS
                            )
                        except GLib.Error as e:
                            print(
                                f"Could not read action file '{full_path}': {e.message}"
                            )
                            continue
                        ...
                        try:
                            ...
                            print(
                                f"Could not read Name field for action. Skipping '{full_path}': {e.message}"
                            )
                            continue

                        try:
                            metadata["description"] = keyfile.get_locale_string(
                                "Nemo Action", "Comment"
                            )
                        except GLib.Error as e:
                            if e.code == GLib.KeyFileError.NOT_FOUND:
                        ...
                        try:
                            metadata["icon"] = keyfile.get_string(
                                "Nemo Action", "Icon-Name"
                            )
                        except GLib.Error as e:
                            if e.code == GLib.KeyFileError.NOT_FOUND:
                                pass
                    ...
                    if self.actions and not os.path.isdir(full_path):
                        ...
                        try:
                            ...
                            with open(
                                f"{full_path}/metadata.json", encoding="utf-8"
                            ) as json_data:
    ...
    def _download_image_cache(self):
        ...
        for uuid, _ in self.index_cache.items():
            if self.themes:
                icon_basename = self._sanitize_thumb(
                    os.path.basename(self.index_cache[uuid]["screenshot"])
                )
                download_url = (
                    URL_SPICES_HOME + "/uploads/themes/thumbs/" + icon_basename
                )
            else:
            ...
            if (
                ...
                or self._is_bad_image(icon_path)
                or self.old_cache[uuid]["last_edited"]
                != self.index_cache[uuid]["last_edited"]
            ):
                self.download_manager.push(
                    self._download,
                    self._check_download_image_cache_complete,
                    (icon_path, download_url),
                )
    ...
    def _sanitize_thumb(basename):
        return (
            basename.replace("jpg", "png").replace("JPG", "png").replace("PNG", "png")
        )
    ...
    def _install(self, job):
        ...
        try:
            with zipfile.ZipFile(ziptempfile) as _zip:
                ...
                uuidfolder = (
                    tempfolder if self.actions else os.path.join(tempfolder, uuid)
                )
    ...
    def _install_finished(self, job):
        ...
        if self.get_enabled(uuid):
            if self._proxy:
                ...
                self.send_proxy_signal(
                    "ReloadXlet", "(ss)", uuid, self.collection_type.upper()
                )
    ...
    def _uninstall(self, job):
        try:
            ...
            if not self.themes:
                ...
                if os.path.exists(locale_inst):
                    ...
                    for i19_folder in i19_folders:
                        if os.path.isfile(
                            os.path.join(
                                locale_inst, i19_folder, "LC_MESSAGES", f"{uuid}.mo"
                            )
                        ):
                            os.remove(
                                os.path.join(
            ...
            self.errorMessage(
                _("A problem occurred while removing %s.") % job["uuid"], str(error)
            )
    ...
    def enable_extension(self, uuid, panel=1, box="right", position=0):
        if self.collection_type == "applet":
            ...
            enabled.append(
                f"{uuid}:{desklet_id}:{primary_rect.x + 100}:{primary_rect.y + 100}"
            )
        ...
        try:
            if self.themes:
                ...
                pixbuf = GdkPixbuf.Pixbuf.new_from_file_at_scale(
                    file_path, 100, -1, True
                )
            else:
                file_path = os.path.join(
                    self.cache_folder, os.path.basename(self.index_cache[uuid]["icon"])
                )
                pixbuf = GdkPixbuf.Pixbuf.new_from_file_at_scale(
                    file_path, 24, 24, True
                )
