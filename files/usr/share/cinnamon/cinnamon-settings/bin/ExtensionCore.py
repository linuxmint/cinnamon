...
def get_cinnamon_version():
    version_str = subprocess.check_output(
        ["cinnamon", "--version"], encoding="utf-8"
    ).split()[1]
...
def translate(uuid, string):
    ...
    if uuid not in translations:
        try:
            translations[uuid] = gettext.translation(
...
class ManageSpicesRow(Gtk.ListBoxRow):
    def __init__(self, extension_type, metadata, size_groups):
        ...
        self.description = translate(
            self.metadata["uuid"], self.metadata["description"]
        )
        ...
        if "author" in metadata:
            if (
                metadata["author"].lower() != "none"
                and metadata["author"].lower() != "unknown"
            ):
        ...
        if not self.metadata.get("hide-configuration"):
            if "external-configuration-app" in self.metadata:
                self.ext_config_app = os.path.join(
                    self.metadata["path"], self.metadata["external-configuration-app"]
                )
            ...
            if self.ext_config_app is None and os.path.exists(
                f"{self.metadata['path']}/settings-schema.json"
            ):
                self.has_config = True
        ...
        if os.path.exists(icon_path):
            try:
                ...
                pixbuf = GdkPixbuf.Pixbuf.new_from_file_at_scale(
                    icon_path, width * scale, width * scale, True
                )
        ...
        if icon is None:
            icon = Gtk.Image.new_from_icon_name(
                f"cs-{extension_type}s", Gtk.IconSize.LARGE_TOOLBAR
            )
        ...
        grid.attach_next_to(
            self.button_box, self.status_box, Gtk.PositionType.RIGHT, 1, 1
        )
        ...
        try:
            ...
            for version in self.metadata["cinnamon-version"]:
                ...
                if spice_ver[:2] <= curr_ver:
                    ...
                    path = os.path.join(
                        self.metadata["path"], self.extension_type + ".js"
                    )
                    if os.path.exists(path):
                        return True
                    print(
                        f"The {self.uuid} {self.extension_type} is not properly structured. Path not found: '{path}'"
                    )
                    return False
            print(
                f"The {self.uuid} {self.extension_type} is not compatible with this version of Cinnamon."
            )
            return False
        except:
    ...
    def scan_extension_for_danger(self, directory):
        background_work_queue.push(
            self.scan_extension_thread, self.on_scan_complete, (directory,)
        )
...
class ManageSpicesPage(SettingsPage):
    def __init__(self, parent, collection_type, spices, window):
        ...
        self.instance_button = Gtk.Button.new_from_icon_name(
            "xsi-list-add-symbolic", Gtk.IconSize.MENU
        )
        ...
        self.remove_button = Gtk.Button.new_from_icon_name(
            "xsi-list-remove-symbolic", Gtk.IconSize.MENU
        )
        ...
        self.restore_button = Gtk.Button.new_from_icon_name(
            "xsi-edit-undo-symbolic", Gtk.IconSize.MENU
        )
        ...
        self.about_button = Gtk.Button.new_from_icon_name(
            "xsi-help-about-symbolic", Gtk.IconSize.MENU
        )
    ...
    def update_button_states(self, *args):
        ...
        if row is None:
            ...
            self.instance_button.set_sensitive(
                row.enabled == 0 or row.max_instances != 1
            )
    ...
    def add_instance(self, *args):
        ...
        self.enable_extension(
            extension_row.uuid, extension_row.name, extension_row.version_supported
        )

    def enable_extension(self, uuid, name, version_check=True):
        if not version_check:
            show_message(
                _("Extension %s is not compatible with your version of Cinnamon.")
                % uuid,
                self.window,
            )
            return
    ...
    def uninstall_extension(self, *args):
        extension_row = self.list_box.get_selected_row()
        if not show_prompt(
            _("Are you sure you want to completely remove %s?") % extension_row.uuid,
            self.window,
        ):
            return
    ...
    def load_extensions(self, *args):
        ...
        size_groups = [
            Gtk.SizeGroup.new(Gtk.SizeGroupMode.HORIZONTAL) for i in range(3)
        ]

        for uuid, metadata in self.spices.get_installed().items():
            try:
                extension_row = ManageSpicesRow(
                    self.collection_type, metadata, size_groups
                )
    ...
    def update_status(self, *args):
        for row in self.extension_rows:
            ...
            if (
                enabled
                and not self.spices.get_is_running(row.uuid)
                and self.collection_type != "action"
            ):
                row.add_status(
                    "error",
...
class DownloadSpicesRow(Gtk.ListBoxRow):
    def __init__(self, uuid, data, spices, size_groups):
        ...
        if "author_user" in data:
            if (
                data["author_user"].lower() != "none"
                and data["author_user"].lower() != "unknown"
            ):
        ...
        if "translations" in data.keys():
            ...
            for key in (
                f"description_{LONG_LANGUAGE_CODE}",
        ...
        if self.spices.get_has_update(uuid):
            ...
            description_label.set_markup(
                f"<small>{description_markup}</small>\n<small><i>{subject_markup}</i></small>"
            )
        else:
            description_label.set_markup(f"<small>{description_markup}</small>")
        description_label.set_margin_top(2)
        ...
        if not self.installed:
            download_button = Gtk.Button.new_from_icon_name(
                "xsi-folder-download-symbolic", 2
            )
...
class DownloadSpicesPage(SettingsPage):
    def __init__(self, parent, collection_type, spices, window):
        ...
        self.more_info_button = Gtk.Button.new_from_icon_name(
            "xsi-dialog-information-symbolic", Gtk.IconSize.MENU
        )
        ...
        self.update_all_button = Gtk.Button.new_from_icon_name(
            "xsi-software-update-available-symbolic", Gtk.IconSize.MENU
        )
        ...
        self.refresh_button = Gtk.Button.new_from_icon_name(
            "xsi-emblem-synchronizing-symbolic", Gtk.IconSize.MENU
        )
    ...
    def build_list(self, *args):
        ...
        size_groups = [
            Gtk.SizeGroup.new(Gtk.SizeGroupMode.HORIZONTAL) for i in range(4)
        ]
        ...
        if updates_available > 0:
            msg_text = (
                ...
                + " ("
                + ngettext(
                    "%d update available", "%d updates available", updates_available
                )
                % updates_available
                + ")"
            )
