...
def spawn(command):
    try:
        process = subprocess.run(
            parse_args(command), stdout=subprocess.PIPE, check=True
        )
    except Exception:
...
def handle_cli():
    if CLI[1] == "get_process":
        ...
        try:
            ...
            icons_dir = "{}/.local/share/icons/hicolor/48x48/apps/".format(
                os.getenv("HOME")
            )
            ...
            try:
                try:
                    spawn(
                        "gnome-exe-thumbnailer {} {}".format(
                            process.split("wine ")[1], icon
                        )
                    )
                except IndexError:
            ...
            desktop_path = "{}/.local/share/applications/{}".format(
                os.getenv("HOME"), desktop_file
            )
