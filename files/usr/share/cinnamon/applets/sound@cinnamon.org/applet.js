...
class Player extends PopupMenu.PopupMenuSection {
    ...
    _dbus_acquired() {
        ...
        let artistIcon = new St.Icon({ icon_type: St.IconType.SYMBOLIC, icon_name: "xapp-users", style_class: 'popup-menu-icon' });
        ...
        let titleIcon = new St.Icon({ icon_type: St.IconType.SYMBOLIC, icon_name: "xapp-audio-x-generic", style_class: 'popup-menu-icon' });
        ...
        this._prevButton = new ControlButton("xapp-media-skip-backward",
                                             _("Previous"),
                                             () => this._mediaServerPlayer.PreviousRemote());
        this._playButton = new ControlButton("xapp-media-playback-start",
                                             _("Play"),
                                             () => this._mediaServerPlayer.PlayPauseRemote());
        this._stopButton = new ControlButton("xapp-media-playback-stop",
                                             _("Stop"),
                                             () => this._mediaServerPlayer.StopRemote());
        this._nextButton = new ControlButton("xapp-media-skip-forward",
                                             _("Next"),
        ...
        this._loopButton = new ControlButton("xapp-media-playlist-consecutive", _("Consecutive Playing"), () => this._toggleLoopStatus());
        this.controls.add_actor(this._loopButton.getActor());

        this._shuffleButton = new ControlButton("xapp-media-playlist-shuffle", _("No Shuffle"), () => this._toggleShuffle());
    ...
    _showCanRaise() {
        let btn = new ControlButton("xapp-go-up", _("Open Player"), () => {
    ...
    _showCanQuit() {
        let btn = new ControlButton("xapp-window-close", _("Quit Player"), () => {
    ...
    _setStatus(status) {
        ...
        if (status == "Playing") {
            this._playButton.setData("xapp-media-playback-pause", _("Pause"));
            this.playerIcon.set_icon_name("xapp-media-playback-start");
            this._applet.setAppletTextIcon(this, true);
            this._seeker.play();
        }
        else if (status == "Paused") {
            this._playButton.setData("xapp-media-playback-start", _("Play"));
            this.playerIcon.set_icon_name("xapp-media-playback-pause");
            this._applet.setAppletTextIcon(this, false);
            this._seeker.pause();
        }
        else if (status == "Stopped") {
            this._playButton.setData("xapp-media-playback-start", _("Play"));
            this.playerIcon.set_icon_name("xapp-media-playback-stop");
            this._applet.setAppletTextIcon(this, false);
            this._seeker.stop();
        } else {
            this._applet.setAppletTextIcon(this, false);
    ...
    _setLoopStatus(status) {
        ...
        if(status === "None")
            this._loopButton.setData("xapp-media-playlist-consecutive-symbolic", _("Consecutive Playing"));
        else if(status === "Track")
            this._loopButton.setData("xapp-media-playlist-repeat-song", _("Repeat Single"));
        else if(status === "Playlist")
            this._loopButton.setData("xapp-media-playlist-repeat", _("Repeat All"));
    ...
    _setShuffle(status) {
        ...
        this._shuffleButton.setData("xapp-media-playlist-shuffle", status? _("Shuffle") : _("No Shuffle"));
...
class CinnamonSoundApplet extends Applet.TextIconApplet {
    constructor(metadata, orientation, panel_height, instanceId) {
        ...
        this.set_applet_icon_symbolic_name('xapp-audio-x-generic');
        ...
        this._outputIcon = "xapp-audio-volume-muted";
        ...
        this.mute_out_switch = new PopupMenu.PopupSwitchIconMenuItem(_("Mute output"), false, "xapp-audio-volume-muted", St.IconType.SYMBOLIC);
        this.mute_in_switch = new PopupMenu.PopupSwitchIconMenuItem(_("Mute input"), false, "xapp-microphone-sensitivity-muted", St.IconType.SYMBOLIC);
    ...
    setAppletIcon(player, path) {
        ...
        if (this.showalbum) {
            if (path && player && (player === true || player._playerStatus == 'Playing')) {
                ...
                this.setIcon('xapp-media-optical-cd-audio', 'player-name');
            }
        }
        else {
            this.setIcon('xapp-audio-x-generic', 'player-name');
        }
    }
    ...
    _readOutput() {
        ...
        if (this._output) {
            ...
            this.setIcon("xapp-audio-volume-muted-symbolic", "output");
        }
    }
