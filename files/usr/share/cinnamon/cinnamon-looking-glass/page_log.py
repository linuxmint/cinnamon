...
class LogEntry:
    def __init__(self, category, time, message):
        ...
        self.time = int(time)
        self.timestr = datetime.datetime.fromtimestamp(self.time).strftime(
            "%Y-%m-%dT%H:%M:%SZ"
        )
...
class LogView(Gtk.ScrolledWindow):
    def __init__(self, proxy):
        ...
        self.scroll_mark = self.textbuffer.create_mark(
        ...
        self.type_tags = {
            "info": self.textbuffer.create_tag(
                "info", invisible=not self.enabled_types["info"], invisible_set=True
            ),
            "warning": self.textbuffer.create_tag(
                "warning",
                foreground_rgba=context.lookup_color("warning_color")[1],
                invisible=not self.enabled_types["warning"],
                invisible_set=True,
            ),
            "error": self.textbuffer.create_tag(
                "error",
                foreground_rgba=context.lookup_color("error_color")[1],
                invisible=not self.enabled_types["error"],
                invisible_set=True,
            ),
            "trace": self.textbuffer.create_tag(
                "trace",
    ...
    def on_status_change(self, proxy, online):
        ...
        if online:
            ...
            entry = self.append(
                "warning", 0, "================ DBus connection lost ==============="
            )
        self.textbuffer.insert_with_tags(
            text_iter, entry.formatted_text, self.type_tags[entry.category]
        )
        ...
        if success:
            try:
                ...
                if data_size > 0:
                    ...
                    if (
                        self.added_messages > data_size
                        or self.first_message_time != first_message_time
                        or self.need_reread
                    ):
...
class ModulePage(pageutils.WindowAndActionBars):
    def __init__(self, parent):
        ...
        self.add_toggle_button(
            "error", "xsi-dialog-error-symbolic", "Show/Hide Messages tagged as 'error'"
        )
        self.add_toggle_button(
            "trace",
