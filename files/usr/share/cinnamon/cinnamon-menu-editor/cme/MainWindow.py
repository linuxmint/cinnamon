            ...
            match self.editor.getIsItemUserOrSystem(item):
                case "user only":
                    res = self._show_message(
                        _("Delete Menu Entry"),
                        _("This will delete '%s' launcher from all menu categories and delete its associated .desktop file. Are you sure?") % item.get_app_info().get_name(),
            ...
            match self.editor.getIsItemUserOrSystem(item):
                case "user only":
                    res = self._show_message(
                        ...
                        _("Delete the menu '%s' and its associated .directory file? Items inside the menu will not be deleted.") % item.get_name(),
                        Gtk.ButtonsType.YES_NO,
                        Gtk.MessageType.WARNING
                    )
                    if res == Gtk.ResponseType.YES:
