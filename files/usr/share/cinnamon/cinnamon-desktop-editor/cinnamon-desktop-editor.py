...
class ItemEditor(object):
    ...
    def pick_exec(self, button):
        ...
        chooser.add_buttons(
            "_Cancel", Gtk.ResponseType.REJECT,
...
class LauncherEditor(ItemEditor):
            ...
            while True:
                ...
                if item_type == CMenu.TreeItemType.DIRECTORY:
                    ...
                    if cat_id and cat_id not in DONT_SHOWS:
                        ...
                        cb.set_visible(True)
                        flowbox.add(cb)
                        cb.get_parent().set_can_focus(False)
                        self.category_widgets[cat_id] = cb
                        cb.connect("toggled", self.on_category_toggled, cat_id)

    def on_category_toggled(self, button, cat_id):
        if not button.get_active():
            return

        # These mutually exclusive categories are based on /etc/xdg/menus/cinnamon-applications.menu
        def uncheck(cat):
            if cat in self.category_widgets:
                self.category_widgets[cat].set_active(False)

        if cat_id == "Accessories":
            uncheck("Universal Access")
            uncheck("Administration")
        elif cat_id == "Universal Access":
            uncheck("Accessories")
            uncheck("Preferences")
        elif cat_id == "Preferences":
            uncheck("Universal Access")
            uncheck("Administration")
        elif cat_id == "Administration":
            uncheck("Preferences")
            uncheck("Accessories")
    ...
    def _clear_menu_overrides(self):
        ...
        try:
            ...
            msg = f"Desktop Editor: Could not update .menu file: {e}"
            print(msg)
            show_error_dialog(msg)
...
class DirectoryEditor(ItemEditor):
    ...
    def check_custom_path(self):
        ...
        if is_system_directory(file_name):
            ...
            # than a user created directory, we show a warning that 'restore' is not available.
            if not file_name.startswith("alacarte-"):
                self.builder.get_object('restore-info-bar').show()
