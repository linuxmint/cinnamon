...
class Module:
    def __init__(self, content_box):
        ...
        advanced = False
        sidePage = DeskletViewSidePage(_("Desklets"), "desklets.svg", keywords, advanced, content_box)
...
class DeskletViewSidePage (SidePage):
    SORT_NAME = 0
    SORT_RATING = 1
    SORT_DATE_EDITED = 2
    SORT_ENABLED = 3
    ...
    def build(self, advanced):
        ...
        self.search_entry.set_placeholder_text(_("Search desklets"))
        self.search_entry.connect('changed', self.on_entry_refilter)

        self.notebook.append_page(desklets_vbox, Gtk.Label(_("Installed")))
        ...
        self.settings.connect("changed::enabled-desklets", lambda x,y: self._enabled_desklets_changed())
        ...
        self.instanceButton.set_tooltip_text(_("Some desklets can be added multiple times.\nUse this to add another instance. Use panel edit mode to remove a single instance."))
        self.instanceButton.set_sensitive(False);

        self.configureButton = Gtk.Button(_("Configure"))
        self.configureButton.connect("clicked", self._configure_desklet)
        self.configureButton.set_tooltip_text(_("Configure this desklet"))

        self.extConfigureButton = Gtk.Button(_("Configure"))
        self.extConfigureButton.connect("clicked", self._external_configure_launch)
        self.extConfigureButton.set_tooltip_text(_("Configure this desklet"))


        restoreButton = Gtk.Button(_("Restore to default"))
        restoreButton.connect("clicked", lambda x: self._restore_default_desklets())
        # Installed 
        hbox = Gtk.HBox()
        ...
        desklets_vbox.pack_start(hbox, False, False, 4)
        hbox.show()
        self.search_entry.show()

        desklets_vbox.pack_start(scrolledWindow, True, True, 0)
        hbox = Gtk.HBox()
        desklets_vbox.pack_start(hbox, False, True, 5)
        ...
        self.gm_search_entry.set_placeholder_text(_("Search desklets"))
        ...
        self.install_button.connect("clicked", lambda x: self.install_desklets())
        self.content_box.show_all()   
        self.treeview.get_selection().connect("changed", lambda x: self._selection_changed());
        self.install_list = []
        
        self.spices = Spice_Harvester('desklets', self.window, self.builder, self.on_enable_new_desklet)
        if not self.spices.get_webkit_enabled():
            getmore_label.set_sensitive(False)
            reload_button.set_sensitive(False)

        self.spices.scrubConfigDirs(self.enabled_desklets)
            ...
            if data:
                ...
                if sel:
                    ...
                    if self.modelfilter.get_value(iter, 2) > 0:
                        ...
                        item.connect('activate', lambda x: self.disable_desklet(uuid))
                        popup.add(item)
                        ...
                        if can_instance:
                            ...
                            item.connect('activate', lambda x: self.instance_desklet(uuid))
                            popup.add(item)
                    else:
                        item = Gtk.MenuItem(_("Add to panel"))
                        item.connect('activate', lambda x: self.enable_desklet(uuid))
                        popup.add(item)
                        
                    
                    item = Gtk.MenuItem(_("Uninstall"))
                    if self.modelfilter.get_value(iter, 6):
                        item.connect('activate', lambda x: self.uninstall_desklet(uuid, name))
    ...
    def only_active(self, model, iterr, data=None):
        ...
        deskletName = model.get_value(iterr, 5)
        ...
        if self.onlyActive == True:
            return enabled > 0 and (query == "" or query in deskletName.lower())
        else:
            return enabled == 0 and (query == "" or query in deskletName.lower())
    ...
    def on_spice_load(self, spicesData):
        ...
        for uuid in spicesData:
            deskletData = spicesData[uuid]
            deskletName = deskletData['name'].replace('&', '&amp;')
            iter = self.gm_model.insert_before(None, None)
            self.gm_model.set_value(iter, 0, uuid)
            self.gm_model.set_value(iter, 1, '<b>%s</b>\n<b><span foreground="#333333" size="xx-small">%s</span></b>\n<i><span foreground="#555555" size="x-small">%s</span></i>' % (deskletName, uuid, "Description not implemented"))
            self.gm_model.set_value(iter, 2, 0)
            
            icon_filename = os.path.basename(deskletData['icon'])
            
            if not os.path.exists(os.path.join(self.spices.get_cache_folder(), icon_filename)):
                img = GdkPixbuf.Pixbuf.new_from_file_at_size( "/usr/lib/cinnamon-settings/data/icons/desklets.svg", 32, 32)
            else:
            ...
            self.gm_model.set_value(iter, 4, int(deskletData['score']))
            self.gm_model.set_value(iter, 5, deskletData['name'])
            self.gm_model.set_value(iter, 6, int(deskletData['last_edited']))

    def enable_desklet(self, uuid):
        desklet_id = self.settings.get_int("next-desklet-id");
        self.settings.set_int("next-desklet-id", (desklet_id+1));
        self.enabled_desklets.append("%s:%d:0:0" % (uuid, desklet_id))
        self.settings.set_strv("enabled-desklets", self.enabled_desklets)

    def disable_desklet(self, uuid):
        newDesklets = []
        for enabled_desklet in self.enabled_desklets:
            if uuid not in enabled_desklet:
                newDesklets.append(enabled_desklet)
        self.enabled_desklets = newDesklets
        self.settings.set_strv("enabled-desklets", self.enabled_desklets)

    def uninstall_desklet(self, uuid, name):
        self.disable_desklet(uuid)
        self.spices.uninstall(uuid, name, self.on_uninstall_finished)
    
    def on_uninstall_finished(self, uuid):
        self.load_desklets()

    def instance_desklet(self, uuid):
        desklet_id = self.settings.get_int("next-desklet-id");
        self.settings.set_int("next-desklet-id", (desklet_id+1));
        self.enabled_desklets.append("%s:%d:0:0" % (uuid, desklet_id))
        self.settings.set_strv("enabled-desklets", self.enabled_desklets)
    ...
    def _enabled_desklets_changed(self):
        last_selection = ''
        ...
        for enabled_desklet in self.enabled_desklets:
            try:
                uuid, instanceId, x, y = enabled_desklet.split(":")
    ...
    def _add_another_instance_iter(self, treeiter):
        ...
        self.instance_desklet(uuid)
        
    def _selection_changed(self):
        model, treeiter = self.treeview.get_selection().get_selected()
        enabled = False;
        
        tip = _("Some desklets can be added multiple times.\nUse this to add another instance. Use panel edit mode to remove a single instance.")
        if treeiter:
            checked = model.get_value(treeiter, 2);
            max_instances = model.get_value(treeiter, 3);
            enabled = max_instances > checked
            if max_instances == 1:
                tip += _("\nThis desklet does not support multiple instances.")
            else:
                tip += _("\nThis desklet supports max %d instances.") % max_instances
    ...
    def _configure_desklet(self, widget):
        model, treeiter = self.treeview.get_selection().get_selected()
        if treeiter:
            uuid = model.get_value(treeiter, 0)
            settingContainer = XletSettings.XletSetting(uuid, self, "desklet")
    ...
    def _restore_default_desklets(self):
        os.system('gsettings reset org.cinnamon next-desklet-id')
        os.system('gsettings reset org.cinnamon enabled-desklets')
    
    def load_desklets(self):
        self.model.clear()
            ...
            for desklet in desklets:
                try:           
                    if os.path.exists("%s/%s/metadata.json" % (directory, desklet)):
                        ...
                        try: desklet_max_instances = int(data["max-instances"])
                        except KeyError: desklet_max_instances = 1
                        except ValueError: desklet_max_instances = 1

                        try: desklet_role = data["role"]
                        except KeyError: desklet_role = None
                        except ValueError: desklet_role = None
                        ...
                        try: ext_config_app = os.path.join(directory, desklet, data["external-configuration-app"])
                        ...
                        if desklet_max_instances < -1:
                            desklet_max_instances = 1
                            
                        if self.search_entry.get_text().upper() in (desklet_name + desklet_description).upper():
                            iter = self.model.insert_before(None, None)
                            found = 0
                            for enabled_desklet in self.enabled_desklets:
                                if desklet_uuid in enabled_desklet:
                                    found += 1

                            self.model.set_value(iter, 0, desklet_uuid)                
                            self.model.set_value(iter, 1, '<b>%s</b>\n<b><span foreground="#333333" size="xx-small">%s</span></b>\n<i><span foreground="#555555" size="x-small">%s</span></i>' % (desklet_name, desklet_uuid, desklet_description))                                  
                            self.model.set_value(iter, 2, found)
                            self.model.set_value(iter, 3, desklet_max_instances)
                            img = None                            
                            if "icon" in data:
                                desklet_icon = data["icon"]
                                theme = Gtk.IconTheme.get_default()                                                    
                                if theme.has_icon(desklet_icon):
                                    img = theme.load_icon(desklet_icon, 32, 0)
                            elif os.path.exists("%s/%s/icon.png" % (directory, desklet)):
                                img = GdkPixbuf.Pixbuf.new_from_file_at_size("%s/%s/icon.png" % (directory, desklet), 32, 32)                            
                            
                            if img is None:                                                
                                img = GdkPixbuf.Pixbuf.new_from_file_at_size( "/usr/lib/cinnamon-settings/data/icons/desklets.svg", 32, 32)
                                
                            self.model.set_value(iter, 4, img)
                            self.model.set_value(iter, 5, desklet_name)
    ...
    def toggled(self, renderer, path, treeview):        
        ...
        if (iter != None):
            ...
            if (checked > 1):
                msg = _("There are multiple instances of this desklet, do you want to remove them all?\n\n")
