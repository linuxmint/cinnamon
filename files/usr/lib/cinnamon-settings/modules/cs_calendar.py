...
class Module:
    def __init__(self, content_box):
        ...
        try:
            ...
            self.changeDateWidget = ChangeDateWidget()
            self.ntpCheckButton = None 
            try:
                sidePage.add_widget(self.timeLabelWidget)
                sidePage.add_widget(self.dateLabelWidget)
            ...
            sidePage.add_widget(self.changeDateWidget)
            try:
                sidePage.add_widget(Gtk.Separator.new(Gtk.Orientation.HORIZONTAL))
            ...
            if self.ntpCheckButton != None:
                ...
                self.changeDateWidget.change_using_ntp( self.ntpCheckButton.get_active() )
        except Exception, detail:
            print detail
...
#Fancy Current Time Label
class TimeLabelWidget(Gtk.HBox):
    def __init__(self):
        super(TimeLabelWidget, self).__init__()
        
        time_box = Gtk.HBox()
        
        self.time_pm_label = Gtk.Label()
        
        time_box.pack_start(self.time_pm_label, False, False, 0)
        
        self.update_time()
        GObject.timeout_add(1000, self.update_time)     
        
        self.pack_start(time_box, True, False, 0)
        
    def update_time(self):
        dt = datetime.now()
        
        time_pm = dt.strftime("%l:%M %p") #Time, e.g. (3:04 PM)
        
        self.time_pm_label.set_markup("<span font_desc='25'><b>%s</b></span>" %(time_pm))
        
        return True

#Another Fancy Label for Dates (Like the ones I haven't been on) ~RavetcoFX
class DateLabelWidget(Gtk.HBox):
    def __init__(self):
        super(DateLabelWidget, self).__init__()
        
        date_box = Gtk.HBox()
        
        self.date_label = Gtk.Label()
        
        date_box.pack_start(self.date_label, False, False, 0)
        
        self.update_time()
        GObject.timeout_add(1000, self.update_time)     
        
        self.pack_start(date_box, True, False, 0)
        
    def update_time(self):
        dt = datetime.now()
        
        date_now = dt.strftime("%A, %B %e, %Y") #Date, e.g. (Sunday, March 16, 2041)
        
        self.date_label.set_markup("<span font_desc='18'>%s</span>" %(date_now))
        
        return True
    
#Change Time Widget
class ChangeTimeWidget(IndentedHBox):
    def __init__(self):
    ...
    def _change_system_time(self, widget):
        if not self.changedOnTimeout:
            dt = datetime.now()
            other = ChangeDateWidget()
            
            #From ChangeDateWidget
            month = other.monthBox.get_active() + 1
            day = int( other.daySpin.get_value() )
            year = int( other.yearSpin.get_value() )
...
#Date Widget
class ChangeDateWidget(IndentedHBox):
    def __init__(self):
        super(ChangeDateWidget, self).__init__()
        self.date_time_wrapper = DateTimeWrapper()
    ...
    def _change_system_time(self, widget):
        if not self.changedOnTimeout:
            ...
            other = ChangeTimeWidget()
            
            month = self.monthBox.get_active() + 1
            day = int( self.daySpin.get_value() )
            year = int( self.yearSpin.get_value() )
            
            #From ChangeTimeWidget
            hour = int( other.hourSpin.get_value() )
            minute = int( other.minSpin.get_value() )
...
class NtpCheckButton(Gtk.CheckButton):
    def __init__(self, label):
        ...
        self.date_time_wrapper = DateTimeWrapper()
        self.set_tooltip_text(_("Syncs local time to a remote server"))
