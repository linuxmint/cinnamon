...
var PopupResizeHandler = class PopupResizeHandler {
    ...
    _motion_event(box, event) {
        ...
        if (this.in_top_resize_zone (x, y) && this.actor.y > this.edges.top) {
            t = true;
            cursor = Cinnamon.Cursor.RESIZE_TOP;
            this._current_resize_direction |= TOP_EDGE_DRAGGABLE;
        } else
        if (this.in_bottom_resize_zone (x, y) && this.actor.y + this.actor.height < this.edges.bottom) {
            b = true;
        ...
        if (this.in_right_resize_zone (x, y) && this.actor.x + this.actor.width < this.edges.right) {
            r = true;
