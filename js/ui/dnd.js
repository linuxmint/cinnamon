...
var _Draggable = new Lang.Class({
        ...
        while (target) {
            if (target._delegate && target._delegate.acceptDrop) {
                ...
                if (target._delegate.acceptDrop(this.actor._delegate,
                    ...
                    // handle it ourselves.
                    if (!isFinalized(this._dragActor) && this._dragActor.get_parent() === Main.uiGroup) {
    ...
    _dragComplete: function() {
        if (!this._actorDestroyed && !isFinalized(this._dragActor))
...
GenericDragItemContainer.prototype = {
    ...
    set childScale(scale) {
        if (isFinalized(this.child)) return;
