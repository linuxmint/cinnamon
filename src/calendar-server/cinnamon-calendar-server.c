...
#define BUS_NAME "org.Cinnamon.CalendarServer"
      ...
      if (until < since)
        {
          g_dbus_method_invocation_return_dbus_error (invocation,
                                                      "org.Cinnamon.CalendarServer.Error.Failed",
      ...
      char **argv)
{
  ...
  opt_context = g_option_context_new ("cinnamon calendar server");
...
print_debug (const gchar *format, ...)
{
  ...
  g_print ("cinnamon-calendar-server[%d]: %s.%03d: %s\n", pid, timebuf, (gint) (now.tv_usec / 1000), s);
  g_free (s);
 out:
  ;
}
